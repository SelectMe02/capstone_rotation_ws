...
변경 반영 사항
1) elevator_btn_front 도착 전까지 yolov8_node를 실행하지 않음
2) elevator_btn_front 도착 시 yolov8_node 실행
3) btn_down active가 2초 이상 유지되면 확정하고 elevator_front로 즉시 강제 이동
4) elevator_btn_inside에서 elevator_btn_under1 active가 2초 이상 유지되면 elevator_inside_for_exit로 즉시 강제 이동
5) 한 번 active 확정된 버튼은 목표 층 도착 전까지 latched active로 간주하여 재확인하지 않음
6) person이 보이고 필요한 버튼이 보이지 않으면 btn_not_perception.mp3 반복 안내
   - 이미 음성이 재생 중이면 새 음성을 재생하지 않음
   - 버튼이 다시 보이면 추가 반복 재생만 중단하고, 이미 재생 중인 음성은 끊지 않음
7) elevator_inside_for_exit 도착 시 yolov8_node 종료
8) 주요 이동 구간마다 robot_for_move.mp3 1회 재생
9) room_front 출발 시 starting_bgm.mp3 1회 재생
10) destination 도착 시 destination.mp3 1회, 5초 뒤 give_snack.mp3 1회 재생

주의
- RealSense 카메라 노드(realsense2_camera)는 이 파일에서 실행하지 않는다.
  별도 터미널/launch에서 먼저 실행해야 한다.
- 이 파일은 yolov8_node만 필요 구간에서 subprocess로 켰다가 끈다.
- 기본 YOLO 입력 토픽은 RealSense 기본 컬러 이미지 토픽인
  /camera/camera/color/image_raw 로 remap한다.
"""
...
class ElevatorDeliveryFinal(Node):
    """
    엘리베이터 버튼 인식 + waypoint/Nav2 + open-loop forced move 통합 미션 노드.
    """

    def __init__(self):
        super().__init__("elevator_delivery_final")
    ...
    def run(self):
        ...
        try:
            ...
            # 3) elevator_btn_front 도착 시 YOLO 실행
            if not self.start_yolov8_node():
                return

            # 4) btn_down active가 보이면 즉시 elevator_front로 강제 이동
            if not self.wait_until_button_active(
                labels=self.front_button_active_labels,
                button_label_for_log="btn_down",
                context_label="elevator_btn_front",
            ):
                return
            ...
            # 8) 기존 5초 대기 제거. elevator_btn_under1 active 인식 시 즉시 exit 대기 위치로 이동
            if not self.wait_until_button_active(
                labels=self.inside_button_active_labels,
                button_label_for_log="elevator_btn_under1",
                context_label="elevator_btn_inside",
            ):
                return
            ...
            # 9) elevator_inside_for_exit 도착 시 YOLO 종료
            ...
            # 14) destination 도착 안내 1회, 5초 뒤 간식 전달 안내 1회
            self.speaker.play_once(
